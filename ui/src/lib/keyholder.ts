 ...
 *  every tab: unlocking in one unlocks the rest. `extendedLifetime` is why a
 *  lone-tab reload does not drop the key. The bytes never enter page scope
 *  and are never written to storage. Where SharedWorker is missing -- Samsung
 *  Internet, older Chrome for Android -- the same operations run in this page:
 *  one unlock per tab, lost on reload.
 */
